...
class RulesController
{
    ...
    private const RULE_TYPES = ['delivery', 'payment', 'communication', 'quality', 'revision', 'legal', 'other'];
    ...
    private function ensureSchema(): void
    {
        try {
            $this->pdo->exec('CREATE TABLE IF NOT EXISTS rules (
                ...
                rule_type VARCHAR(40) NOT NULL DEFAULT "delivery",
    ...
    {
        $type = strtolower($this->sanitizeText((string) ($payload['rule_type'] ?? $payload['type'] ?? 'delivery')));
        ...
        if (isset($payload['rules']) && is_array($payload['rules'])) {
            ...
            foreach ($titles as $index => $title) {
                ...
                $rules[] = $this->normalizeRulePayload([
                    'title' => (string) $title,
                    'rule_type' => (string) ($types[$index] ?? 'delivery'),
