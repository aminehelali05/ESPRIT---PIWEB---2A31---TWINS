...
class MarketplaceController
{
    ...
    public function handleStripeSuccess(string $sessionId, int $orderId): bool
    {
        ...
        try {
            ...
            $upd = $this->db->prepare(
                "UPDATE marketplace_orders SET status = 'paid', stripe_payment_intent = :pi WHERE id = :id"
            );
            $upd->execute([
        ...
        if ($event->type === 'checkout.session.completed') {
            ...
            if ($orderId > 0) {
                try {
                    $this->db->beginTransaction();
                    $upd = $this->db->prepare(
                        "UPDATE marketplace_orders SET status = 'paid', stripe_payment_intent = :pi 
                         WHERE id = :id AND status = 'pending'"
                    );
                    $upd->execute(['pi' => $session->payment_intent ?? '', 'id' => $orderId]);
                    $this->db->commit();
                } catch (\Throwable $e) {
                    if ($this->db->inTransaction()) $this->db->rollBack();
