...
include_once(__DIR__ . '/TacheController.php');
...
class ProjectController
{
    ...
    public function __construct(?PDO $pdo = null)
    {
        ...
        $this->tacheController = new TacheController($this->pdo);
    }
    ...
    public function listBackofficeRows(): array
    {
        $sql = 'SELECT p.*, u.first_name, u.last_name,
                (SELECT COUNT(*) FROM taches t WHERE t.projet_id = p.id) AS tasks_count
                FROM projects p
    ...
    {
        return $this->tacheController->listByProject($projectId);
    }

    public function createTask(array $payload): int
    {
        $projectId = (int) ($payload['projet_id'] ?? 0);
        if ($projectId <= 0) {
        ...
        return $this->tacheController->create($projectId, [
    ...
    {
        return $this->tacheController->update($taskId, $payload);
    }

    public function updateTaskForOwner(int $taskId, int $ownerId, array $payload): bool
    {
        $task = $this->pdo->prepare('SELECT t.id FROM taches t INNER JOIN projects p ON p.id = t.projet_id WHERE t.id = :id AND p.owner_id = :owner_id LIMIT 1');
    ...
    {
        return $this->tacheController->delete($taskId);
    }
    ...
    {
        $stmt = $this->pdo->prepare('SELECT t.*, p.title AS project_title, p.owner_id FROM taches t INNER JOIN projects p ON p.id = t.projet_id WHERE t.id = :id LIMIT 1');
    ...
    public function tasksMapForProjects(array $projectIds): array
    {
        ...
        $stmt = $this->pdo->prepare('SELECT * FROM taches WHERE projet_id IN (' . $placeholders . ') ORDER BY deadline ASC, id DESC');
        $stmt->execute($projectIds);
        $grouped = [];
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $task) {
            $projectId = (int) ($task['projet_id'] ?? 0);
