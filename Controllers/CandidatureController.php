...
class CandidatureController
{
    ...
    private function ensureSchema(): void
    {
        try {
            $this->pdo->exec('CREATE TABLE IF NOT EXISTS candidatures (
                ...
                UNIQUE KEY uq_candidatures_offer_freelancer (offre_id, freelancer_id),
                KEY idx_candidatures_offer_id (offre_id),
                KEY idx_candidatures_freelancer_id (freelancer_id),
                CONSTRAINT fk_candidatures_offer FOREIGN KEY (offre_id) REFERENCES job_offers(id) ON DELETE CASCADE,
    ...
    {
        try {
            $sql = 'SELECT a.job_offer_id, a.freelancer_id, COALESCE(NULLIF(TRIM(a.cover_letter), ""), CONCAT("Application from freelancer #", a.freelancer_id)) AS message
                    FROM job_offer_applications a
                    LEFT JOIN candidatures c ON c.offre_id = a.job_offer_id AND c.freelancer_id = a.freelancer_id
                    WHERE c.id IS NULL';
            ...
            $insert = $this->pdo->prepare('INSERT INTO candidatures (offre_id, freelancer_id, message, created_at, updated_at)
                VALUES (:offre_id, :freelancer_id, :message, NOW(), NOW())');
            foreach ($rows as $row) {
    ...
    public function create(int $offreId, int $freelancerId, string $message): int
    {
        ...
        $stmt = $this->pdo->prepare('INSERT INTO candidatures (offre_id, freelancer_id, message, created_at, updated_at)
            VALUES (:offre_id, :freelancer_id, :message, NOW(), NOW())');
        $stmt->execute([
            'offre_id' => $offreId,
    ...
    {
        $stmt = $this->pdo->prepare('SELECT c.*, u.first_name, u.last_name, u.email
            FROM candidatures c
    ...
    {
        $stmt = $this->pdo->prepare('SELECT * FROM candidatures WHERE offre_id = :offre_id AND freelancer_id = :freelancer_id LIMIT 1');
        $stmt->execute([
            'offre_id' => $offreId,
            'freelancer_id' => $freelancerId,
        ]);
