...
PROMPT;
    ...
    private function fallbackPlan(string $message, array $context): array
    {
        ...
        if (preg_match('/\b(send|message|chat|reply)\b/', $normalized)) {
            ...
            $body = $this->extractMessageBodyFromMessage($message);
            $steps = [
                [
                    'action' => 'navigate',
                    'target' => 'messages.php',
    ...
    {
        if (preg_match('/\b(?:to|with|for)\s+([a-z0-9_.@ -]{2,})/i', $message, $matches)) {
            return trim((string) ($matches[1] ?? ''));
        }
        return '';
    }

    private function normalizeCommandText(string $message): string
    {
        return strtolower(trim(preg_replace('/\s+/', ' ', $message)));
    }

    private function resolvePageTargetFromMessage(string $message): string
    {
        $normalized = $this->normalizeCommandText($message);
        if ($normalized === '') {
            return '';
        ...
        if (preg_match('/\b(agent|assistant|ai|voice)\b/', $normalized)) {
            return 'Views/FrontOffice/ai-agent.php';
        }

        if (preg_match('/\b(home|dashboard)\b/', $normalized)) {
            return 'Views/FrontOffice/home.php';
        }

        if (preg_match('/\b(profile|account|settings)\b/', $normalized)) {
            return 'Views/FrontOffice/profile.php';
        }

        return '';
    }

    private function extractMessageBodyFromMessage(string $message): string
    {
        ...
        $trimmed = preg_replace('/^(?:send|message|chat|reply)\s+(?:please\s+)?/i', '', $trimmed) ?? $trimmed;
        $trimmed = preg_replace('/\s+(?:to|for|with)\s+[a-z0-9_.@ -]{2,}$/i', '', $trimmed) ?? $trimmed;
