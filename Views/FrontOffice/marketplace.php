...
/* ── Handle AJAX AI Video Generation ────────── */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_ai_video') {
    header('Content-Type: application/json');
    try {
        $title = trim($_POST['title'] ?? '');
        $description = trim($_POST['description'] ?? '');
        $imageData = $_POST['image_data'] ?? null; // Base64 or URL

        if ($title === '') throw new Exception('Product title is required for AI generation.');

        require_once __DIR__ . '/../../services/OpenRouterService.php';
        $orService = new OpenRouterService(config::getConnexion());

        // Use a more powerful model for marketing logic
        $model = "google/gemini-2.0-flash-001"; 

        $messages = [
            ['role' => 'system', 'content' => 'You are an expert high-conversion direct-response marketing director. Your goal is to sell products through short, punchy videos.']
        ];

        $userContent = [
            ['type' => 'text', 'text' => "Create a high-converting marketing video plan for this product.\n\nProduct: {$title}\nDescription: {$description}\n\nRequirements:\n1. 3-sentence script that follows: Attention (Hook) -> Benefit -> Call to Action.\n2. 3 specific scenes that visually demonstrate the product value.\n3. A specific 'mood' (e.g. Luxury, Urban, Tech, Cozy).\n4. Primary and secondary brand colors in hex format.\n\nReturn ONLY a JSON object with keys: \"script\", \"scenes\" (array of 3), \"mood\", \"color_primary\", \"color_secondary\"."]
        ];
        ...
        $aiResult = $orService->callChat($messages, [
            'model' => $model,
            'temperature' => 0.8,
            'max_tokens' => 1000,
        ...
        if (isset($aiResult['choices'][0]['message']['content'])) {
            $aiText = trim($aiResult['choices'][0]['message']['content']);
        } elseif (isset($aiResult['error'])) {
            throw new Exception('AI API error: ' . ($aiResult['error']['message'] ?? $aiResult['error']));
        }

        $aiText = preg_replace('/^```(?:json)?\s*/i', '', $aiText);
        $aiText = preg_replace('/\s*```$/', '', $aiText);

        $videoData = json_decode($aiText, true);
        if (!$videoData || !isset($videoData['script'])) {
            throw new Exception('AI failed to generate valid marketing data.');
        }
...
.mp-stock-badge {
  ...
  padding: 4px 10px; border-radius: var(--mp-r-md);
  font-size: .65rem; font-weight: 800; text-transform: uppercase;
  letter-spacing: .05em; z-index: 10;
  box-shadow: var(--mp-shadow-sm);
  backdrop-filter: blur(8px);
}
.mp-stock-badge.out { background: rgba(244,63,94,0.9); color: #fff; }
.mp-stock-badge.low { background: rgba(245,158,11,0.9); color: #fff; }
...
.mp-video-type-btn.mp-ai-gen-btn {
  display: inline-flex; align-items: center; gap: 5px;
  background: linear-gradient(135deg, rgba(99,102,241,0.08), rgba(168,85,247,0.08));
  color: var(--mp-indigo-600);
}
.mp-video-type-btn.mp-ai-gen-btn:hover {
  background: linear-gradient(135deg, rgba(99,102,241,0.15), rgba(168,85,247,0.15));
}
.mp-video-type-btn.mp-ai-gen-btn.generating {
...
.mp-ai-gen-icon {
  ...
  background: linear-gradient(135deg, var(--mp-indigo-500), #a855f7);
  display: flex; align-items: center; justify-content: center;
...
.mp-ai-gen-title { font-size: .85rem; font-weight: 700; color: var(--mp-gray-900); }
.mp-ai-gen-status { font-size: .72rem; color: var(--mp-indigo-500); font-weight: 500; margin-top: 2px; }

/* Progress bar */
.mp-ai-gen-progress-track {
  height: 6px; background: rgba(99,102,241,0.1);
  border-radius: 3px; overflow: hidden; margin-bottom: 16px;
}
.mp-ai-gen-progress-bar {
  height: 100%; border-radius: 3px;
  background: linear-gradient(90deg, var(--mp-indigo-500), #a855f7, var(--mp-indigo-500));
  background-size: 200% 100%;
  animation: aiBarShimmer 1.5s linear infinite;
  transition: width 0.6s cubic-bezier(0.4,0,0.2,1);
}
@keyframes aiBarShimmer { 0% { background-position: 200% 0; } 100% { background-position: -200% 0; } }
...
.mp-ai-gen-phase {
  ...
  font-size: .68rem; font-weight: 600; color: var(--mp-gray-400);
  padding: 8px 6px; border-radius: var(--mp-r-sm);
  background: rgba(0,0,0,0.02); transition: all 0.3s ease;
}
.mp-ai-gen-phase svg { width: 14px; height: 14px; }
.mp-ai-gen-phase.active {
  color: var(--mp-indigo-600); background: rgba(99,102,241,0.08);
  animation: aiFadeIn 0.3s ease;
}
.mp-ai-gen-phase.done { color: #10b981; background: rgba(16,185,129,0.06); }
@keyframes aiFadeIn { from { opacity: 0; transform: scale(0.95); } to { opacity: 1; transform: scale(1); } }

/* Scenes list */
.mp-ai-gen-scenes { margin-top: 14px; }
.mp-ai-gen-scenes-title { font-size: .7rem; font-weight: 700; color: var(--mp-gray-500); text-transform: uppercase; letter-spacing: .08em; margin-bottom: 8px; }
.mp-ai-gen-scene-item {
  display: flex; align-items: center; gap: 8px;
  font-size: .75rem; color: var(--mp-gray-700); padding: 6px 0;
  border-bottom: 1px solid rgba(0,0,0,0.04);
  animation: aiFadeIn 0.3s ease;
}
.mp-ai-gen-scene-item span {
...
.mp-ai-video-result {
  ...
  animation: aiPanelIn 0.4s ease;
}
.mp-ai-result-header {
  display: flex; justify-content: space-between; align-items: center;
  padding: 12px 16px; font-size: .78rem; font-weight: 700; color: var(--mp-indigo-600);
}
.mp-ai-result-header i { width: 14px; height: 14px; }
.mp-ai-result-badge {
...
.mp-ai-result-script {
  padding: 12px 16px; font-size: .75rem; font-style: italic;
  color: var(--mp-gray-600); line-height: 1.5;
  border-top: 1px solid rgba(99,102,241,0.08);
  border-bottom: 1px solid rgba(99,102,241,0.08);
}
.mp-ai-result-actions {
  display: flex; gap: 8px; padding: 12px 16px;
}
.uf-btn-sm { padding: 6px 14px !important; font-size: .72rem !important; }
.mp-btn-danger { color: #ef4444 !important; }
.mp-btn-danger:hover { background: rgba(239,68,68,0.06) !important; }
        ...
        <?php else: ?>
          ...
          <div class="mp-card reveal-section" role="listitem"
            ...
            <div class="mp-card-preview">
              ...
              <?php if (isset($item['quantity']) && (int)$item['quantity'] <= 0): ?>
                <div class="mp-stock-badge out">Out of Stock</div>
              <?php elseif (isset($item['quantity']) && (int)$item['quantity'] <= 5): ?>
                <div class="mp-stock-badge low">Only <?= (int)$item['quantity'] ?> left</div>
              <?php endif; ?>
              <div class="mp-card-actions">
                <button class="mp-action-btn primary" title="Quick buy"
                  <?php if (isset($item['quantity']) && (int)$item['quantity'] <= 0) echo 'disabled style="opacity:0.5; cursor:not-allowed;"'; ?>
                  onclick="event.stopPropagation(); selectedItemId=<?= (int)$item['id'] ?>; initCheckout()">
                  <i data-lucide="shopping-cart"></i> <?= (isset($item['quantity']) && (int)$item['quantity'] <= 0) ? 'Sold Out' : 'Buy' ?>
                </button>
    ...
    <div class="uf-body">
      <form id="createListingForm" autocomplete="off" onsubmit="handleCreateSubmit(event)" novalidate>
        ...
        <div class="uf-grid">
          ...
          <div class="uf-group" id="field-quantity">
            <label class="uf-label" for="lQuantity">Stock Quantity <span class="required">*</span></label>
            <input type="number" id="lQuantity" name="quantity" class="uf-input" placeholder=" " min="1" step="1" value="1">
            <span class="mp-field-err">Quantity must be at least 1.</span>
          </div>
          <div class="uf-group">
          ...
          <div class="uf-group uf-span-2" id="field-video">
            ...
            <div class="mp-video-type-toggle">
              ...
              <button type="button" class="mp-video-type-btn mp-ai-gen-btn" id="aiGenBtn" onclick="generateAISuggestedVideo()">
                <i data-lucide="sparkles" style="width:14px;height:14px;"></i> AI Generation
              </button>
            </div>
            ...
            <div id="aiGenPanel" class="mp-ai-gen-panel" style="display:none;">
              <div class="mp-ai-gen-header">
                <div class="mp-ai-gen-icon"><i data-lucide="cpu"></i></div>
                <div>
                  <div class="mp-ai-gen-title">AI Video Generator</div>
                  <div class="mp-ai-gen-status" id="aiGenStatus">Initializing...</div>
                </div>
              </div>
              <div class="mp-ai-gen-progress-track">
                <div class="mp-ai-gen-progress-bar" id="aiGenBar" style="width:0%"></div>
              </div>
              <div class="mp-ai-gen-phases" id="aiGenPhases">
                <div class="mp-ai-gen-phase" id="aiPhase1"><i data-lucide="file-text"></i> Analyzing product</div>
                <div class="mp-ai-gen-phase" id="aiPhase2"><i data-lucide="pen-tool"></i> Writing script</div>
                <div class="mp-ai-gen-phase" id="aiPhase3"><i data-lucide="film"></i> Rendering video</div>
                <div class="mp-ai-gen-phase" id="aiPhase4"><i data-lucide="check-circle"></i> Finalizing</div>
              </div>
              <div class="mp-ai-gen-scenes" id="aiGenScenes" style="display:none;"></div>
            </div>

            <!-- AI Generated Video Result -->
            <div id="aiVideoResult" class="mp-ai-video-result" style="display:none;">
              <div class="mp-ai-result-header">
                <span><i data-lucide="sparkles"></i> AI-Generated Preview</span>
                <span class="mp-ai-result-badge">AI</span>
              </div>
              <canvas id="aiVideoCanvas" width="640" height="360" style="width:100%;border-radius:8px;"></canvas>
              <div class="mp-ai-result-script" id="aiResultScript"></div>
              <div class="mp-ai-result-actions">
                <button type="button" class="uf-btn uf-btn-primary uf-btn-sm" onclick="keepAIVideo()"><i data-lucide="check"></i> Keep & Export</button>
                <button type="button" class="uf-btn uf-btn-ghost uf-btn-sm" onclick="generateAISuggestedVideo()"><i data-lucide="refresh-cw"></i> Regenerate</button>
                <button type="button" class="uf-btn uf-btn-ghost uf-btn-sm mp-btn-danger" onclick="removeAIVideo()"><i data-lucide="trash-2"></i> Remove</button>
              </div>
            </div>
    ...
    <div class="uf-body">
      <form id="editListingForm" autocomplete="off" onsubmit="handleEditSubmit(event)" novalidate>
        ...
        <div class="uf-grid">
          ...
          <div class="uf-group" id="efield-quantity">
            <label class="uf-label" for="eQuantity">Stock Quantity <span class="required">*</span></label>
            <input type="number" id="eQuantity" name="quantity" class="uf-input" placeholder=" " min="0" step="1">
            <span class="mp-field-err">Quantity cannot be negative.</span>
          </div>
          <div class="uf-group">
...
(function () {
  ...
  function initMarketplace() {
    try {
      ...
      initDragDrop();
      initDeliveryToggle();
      initRealTimeValidation('createListingForm');
      initRealTimeValidation('editListingForm');
      initFloatingLabels();
  ...
  function normalizeItem(item) {
    ...
    m.quantity      = item && item.quantity !== undefined ? Number(item.quantity) : (base.quantity !== undefined ? Number(base.quantity) : 1);
    return m;
  }
  ...
  function updateBuyBtn(deliv) {
    ...
    } else if (deliv.quantity <= 0) {
      btn.disabled = true;
      btn.innerHTML = '<i data-lucide="alert-circle"></i> Out of stock';
    } else {
      btn.disabled = false;
  ...
  window.openCreateModal = function() { 
    ...
    syncUfValues('createListingForm');
  };
  ...
  window.openEditModal = function(item) {
    ...
    if (el('eQuantity'))     el('eQuantity').value     = m.quantity;
  ...
  window.generateAISuggestedVideo = function() {
    ...
    const imageData = (thumbImg && thumbImg.src && thumbImg.src.startsWith('data:image')) ? thumbImg.src : null;

    if (!title) { showToast('Enter a product title first so AI can generate a relevant video.', 'error'); return; }

    const panel   = document.getElementById('aiGenPanel');
    const result  = document.getElementById('aiVideoResult');
    const bar     = document.getElementById('aiGenBar');
    const status  = document.getElementById('aiGenStatus');
    const scenes  = document.getElementById('aiGenScenes');
    const btn     = document.getElementById('aiGenBtn');
    if (!panel || !bar || !status) return;

    // Hide other sections, show progress
    document.getElementById('createVideoUploadSection').style.display = 'none';
    document.getElementById('createVideoUrlSection').style.display = 'none';
    ...
    // Reset phases
    for (let i = 1; i <= 4; i++) {
      const ph = document.getElementById('aiPhase' + i);
    ...
    // Phase progression
    const phases = [
      { id: 1, pct: 15, text: 'Analyzing product and image...' },
      { id: 2, pct: 40, text: 'AI is crafting high-conversion copy...' },
      { id: 3, pct: 75, text: 'Rendering professional preview...' },
      { id: 4, pct: 95, text: 'Polishing marketing assets...' }
    ];
    let phaseIdx = 0;
    function advancePhase() {
      if (phaseIdx > 0) {
        const prev = document.getElementById('aiPhase' + phases[phaseIdx - 1].id);
        if (prev) { prev.classList.remove('active'); prev.classList.add('done'); }
      }
      if (phaseIdx < phases.length) {
        const cur = phases[phaseIdx];
        const el = document.getElementById('aiPhase' + cur.id);
        if (el) el.classList.add('active');
        bar.style.width = cur.pct + '%';
        status.textContent = cur.text;
        phaseIdx++;
      }
    }
    ...
    const phaseTimer = setInterval(() => {
      if (phaseIdx < 2) advancePhase();
    }, 800);

    // Call backend
    const fd = new FormData();
    fd.append('action', 'generate_ai_video');
    ...
    fetch('marketplace.php', { method: 'POST', body: fd })
      ...
      .then(data => {
        ...
        if (scenes && vd.scenes) {
          ...
          scenes.innerHTML = '<div class="mp-ai-gen-scenes-title">Marketing Script</div>' +
            vd.scenes.map((s, i) => `<div class="mp-ai-gen-scene-item"><span>${i+1}</span>${esc(s)}</div>`).join('');
        }

        // Render canvas video
        setTimeout(() => {
          advancePhase(); // Phase 4
          renderAIVideoCanvas(vd, title, imageData);
          bar.style.width = '100%';
          status.textContent = 'Marketing video ready!';
          ...
          setTimeout(() => {
            ...
            if (result) {
              ...
              const scriptEl = document.getElementById('aiResultScript');
              if (scriptEl) scriptEl.textContent = '"' + (vd.script || '') + '"';
            }
            if (btn) { btn.disabled = false; btn.classList.remove('generating'); }
            if (window.lucide) lucide.createIcons();
            showToast('AI marketing video generated! Review it below.', 'success');
          }, 600);
        }, 800);
      })
      .catch(err => {
  ...
  /* ── AI Canvas Video Renderer ─────────────────────── */
  let _aiAnimFrame = null;
  let _aiVideoData = null;
  function renderAIVideoCanvas(vd, title, imageData) {
    _aiVideoData = vd;
    const canvas = document.getElementById('aiVideoCanvas');
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    const W = canvas.width, H = canvas.height;
    const c1 = vd.color_primary || '#6366f1';
    const c2 = vd.color_secondary || '#a855f7';
    const sceneTexts = vd.scenes || ['Product', 'Features', 'Get yours'];
    const mood = vd.mood || 'energetic';
    ...
    const particles = Array.from({length: 40}, () => ({
      ...
      r: Math.random() * 4 + 1, a: Math.random() * 0.4 + 0.1
    }));

    if (_aiAnimFrame) cancelAnimationFrame(_aiAnimFrame);
    ...
    function draw(now) {
      ...
      const sceneIdx = Math.floor(cycleElapsed / sceneDuration);
      const sceneProg = (cycleElapsed % sceneDuration) / sceneDuration;
      ...
      const grad = ctx.createRadialGradient(gx, gy, 0, W/2, H/2, W);
      grad.addColorStop(0, c1 + '33'); // 20% opacity
      ...
      if (productImg && productImg.complete) {
        ...
        // Image position/motion based on scene
        let targetX = W/2;
        let targetY = H/2;
        let opacity = 0.8;

        if (sceneIdx === 0) { // Intro - Slide from left
          targetX = W/2 - 50 + (sceneProg * 50);
          opacity = Math.min(sceneProg * 4, 0.8);
        } else if (sceneIdx === 2) { // Outro - Fade and scale
          scale += (sceneProg * 0.1);
          opacity = Math.max(0, 0.8 - (sceneProg - 0.8) * 5);
        }
        ...
        const imgGlow = ctx.createRadialGradient(targetX, targetY + dh/2, 0, targetX, targetY + dh/2, dw/2);
        imgGlow.addColorStop(0, c1 + '44');
        imgGlow.addColorStop(1, 'rgba(0,0,0,0)');
        ctx.fillStyle = imgGlow;
        ctx.globalAlpha = opacity;
        ctx.fillRect(targetX - dw, targetY, dw*2, dh);
        ...
        ctx.drawImage(productImg, targetX - dw/2, targetY - dh/2, dw, dh);
        ctx.restore();
      }
      ...
      // Scene Text (Marketing focus)
      const fadeIn = Math.min(sceneProg * 6, 1);
      const fadeOut = Math.max(0, 1 - (sceneProg - 0.8) * 5);
      const alpha = Math.min(fadeIn, fadeOut);

      ctx.save();
      ctx.globalAlpha = alpha;

      // Text Background Blur/Glow
      ctx.shadowColor = c1;
      ctx.shadowBlur = 15;
      ...
      const textY = H * 0.82;
      ctx.fillText(sceneTexts[sceneIdx]?.toUpperCase() || '', W/2, textY);

      // Sub-badge (e.g. "LIMITED EDITION" or "BEST SELLER")
      ctx.shadowBlur = 0;
      const badgeText = sceneIdx === 0 ? 'NEW ARRIVAL' : (sceneIdx === 1 ? 'TOP RATED' : 'LIMITED OFFER');
      ctx.font = '700 12px Poppins, sans-serif';
      const tw = ctx.measureText(badgeText).width + 20;
      ctx.fillStyle = c2;
      ctx.beginPath();
      ctx.roundRect(W/2 - tw/2, textY - 65, tw, 22, 11);
      ctx.fill();
      ctx.fillStyle = '#fff';
      ctx.fillText(badgeText, W/2, textY - 50);

      ctx.restore();

      // "BUY NOW" Sticker (Always visible, pulsing)
      if (sceneIdx === 2) {
        ctx.save();
        const sPulse = 1 + Math.sin(elapsed * 5) * 0.05;
        ctx.translate(W - 80, 80);
        ctx.rotate(Math.sin(elapsed * 2) * 0.1);
        ...
        ctx.textAlign = 'center';
        ctx.fillText('GET IT', 0, -5);
        ctx.fillText('NOW', 0, 15);
        ctx.restore();
      }

      // Progress bar at bottom
      const pbH = 4, pbY = H - pbH;
      ctx.fillStyle = 'rgba(255,255,255,0.1)';
      ctx.fillRect(0, pbY, W, pbH);
      ctx.fillStyle = c1;
      ctx.fillRect(0, pbY, W * (cycleElapsed / totalDuration), pbH);

      _aiAnimFrame = requestAnimationFrame(draw);
    }
    _aiAnimFrame = requestAnimationFrame(draw);
  }

  /* ── Keep AI Video (export canvas to WebM blob) ───── */
  window.keepAIVideo = function() {
    const canvas = document.getElementById('aiVideoCanvas');
    if (!canvas) return;
    showToast('Exporting AI video preview...', 'info');

    // Record canvas as WebM for ~6 seconds (2 full scene cycles)
    const stream = canvas.captureStream(30);
    const recorder = new MediaRecorder(stream, { mimeType: 'video/webm; codecs=vp9' });
    const chunks = [];
    recorder.ondataavailable = e => { if (e.data.size) chunks.push(e.data); };
    recorder.onstop = () => {
      if (_aiAnimFrame) { cancelAnimationFrame(_aiAnimFrame); _aiAnimFrame = null; }
      const blob = new Blob(chunks, { type: 'video/webm' });
      const file = new File([blob], 'ai_video_' + Date.now() + '.webm', { type: 'video/webm' });

      // Inject into the file input
      const dt = new DataTransfer();
      dt.items.add(file);
      const fileInput = document.getElementById('lVideo');
      ...
      // Hide AI result, show upload section
      document.getElementById('aiVideoResult').style.display = 'none';
      document.getElementById('createVideoUploadSection').style.display = 'block';
      switchVideoType('create', 'upload');
      showToast('AI video exported and attached to your listing! 🎬', 'success');
    };
    recorder.start();
  ...
  /* ── Remove AI Video ──────────────────────────────── */
  window.removeAIVideo = function() {
    if (_aiAnimFrame) { cancelAnimationFrame(_aiAnimFrame); _aiAnimFrame = null; }
    document.getElementById('aiVideoResult').style.display = 'none';
    document.getElementById('aiGenPanel').style.display = 'none';
    document.getElementById('createVideoUploadSection').style.display = 'block';
    switchVideoType('create', 'upload');
    showToast('AI video removed.', 'info');
  };
  ...
  /* ── Floating Labels State Management ──────────────── */
  function initFloatingLabels() {
    ['input', 'blur', 'change'].forEach(evt => {
      document.addEventListener(evt, e => {
        if (e.target.classList && e.target.classList.contains('uf-input')) updateUfGroupState(e.target);
      }, true);
    });
  }

  function updateUfGroupState(input) {
    const group = input.closest('.uf-group');
    if (!group) return;
    const hasVal = String(input.value || '').trim() !== '';
    group.classList.toggle('has-value', hasVal);
  }

  window.syncUfValues = function(formId) {
    const form = document.getElementById(formId);
    if (!form) return;
    form.querySelectorAll('.uf-input').forEach(input => updateUfGroupState(input));
  };
  ...
  window.initRealTimeValidation = function(formId) {
    const form = document.getElementById(formId);
    ...
    const rules = {
      ...
      'eTitle':        (v) => v.trim().length >= 5,
      'lDesc':         (v) => v.trim().length >= 20,
      'eDesc':         (v) => v.trim().length >= 20,
      'lPrice':        (v) => !isNaN(parseFloat(v)) && parseFloat(v) >= 0.5,
      'ePrice':        (v) => !isNaN(parseFloat(v)) && parseFloat(v) >= 0.5,
      'lDeliveryTime': (v) => {
        if (!v) return true;
        const today = new Date().toISOString().split('T')[0];
        return v >= today;
    ...
    const fieldMap = {
      ...
      'eDesc': 'efield-description',
      'lPrice': 'field-price',
      'ePrice': 'efield-price',
      'lDeliveryTime': 'field-delivery-time',
      'eDeliveryTime': 'efield-delivery-time'
    };
