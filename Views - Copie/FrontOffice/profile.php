...
if (isset($_GET['action']) && strtolower((string) $_GET['action']) === 'profile_create_story') {
  ...
  $payload = [
    ...
    'content_json' => $jsonInput['content_json'] ?? null,
...
if (isset($_GET['action']) && strtolower((string) $_GET['action']) === 'profile_get_stories') {
  ...
  $storyExtraColumns = [];
  foreach (['music_url', 'music_title', 'drawing_data', 'text_layers', 'sticker_layers', 'filter_css', 'gradient_bg', 'duration', 'visibility', 'content_json'] as $storyColumn) {
  ...
  try {
    ...
    foreach ((array) $q->fetchAll() as $row) {
      $stories[] = [
        ...
        'content_json' => isset($row['content_json']) && $row['content_json'] !== null ? (json_decode((string) $row['content_json'], true) ?: []) : [],
        'visibility' => (string) ($row['visibility'] ?? 'public'),
        'user' => [
